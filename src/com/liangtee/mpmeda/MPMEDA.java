...
public class MPMEDA {
    ...
    private int lamda;
    private Map<Integer, List<MetaCluster>> mapTree = new HashMap<Integer, List<MetaCluster>>();
    ...
    public Map<Integer, List<MetaCluster>> launch_pro2(double dist_threshold, int data_dim) {
        ...
        for (Entry entry : this.dataSet.getDataSet().entrySet()) {
        ...
        this.mapTree.put(this.layer, floorLayer);

        System.out.println("### inital clustering finished ... ###");

        Set<Integer> passedClusterIDs = new HashSet<Integer>();
        int clusterNum = floorLayer.size();

        int preCurrentLayerClusterNum = 0;
        while (true) {
            List<MetaCluster> newLayerClusters = new ArrayList<MetaCluster>();
            List<MetaCluster> currentLayerClusters = this.mapTree.get(this.layer);
            ...
            for (int i = 0; i < currentLayerClusters.size(); i++) {
                if (!passedClusterIDs.contains(currentLayerClusters.get(i).getCID())) {
                    MetaCluster cluster = currentLayerClusters.get(i);
                    ...
                    for (int j = 0; j < currentLayerClusters.size(); j++) {
                        if ((i != j) && (!passedClusterIDs.contains(currentLayerClusters.get(j).getCID()))) {
                            double[] ME2 = getME(currentLayerClusters.get(j), data_dim);
                    ...
                    if (min_dist <= dist_threshold) {
                        MetaCluster mostSimilarCluster = currentLayerClusters.get(mostSimilarClusterID);
                        clusterNum++;
                        ...
                        newCluster.getSubCluserIDs().add(cluster.getCID());
                        newCluster.getSubCluserIDs().add(mostSimilarCluster.getCID());
                        newLayerClusters.add(newCluster);
                        passedClusterIDs.add(cluster.getCID());
                        passedClusterIDs.add(mostSimilarCluster.getCID());
                    } else {
